...
CREATE TABLE `product` (
  ...
  `min_original_price` decimal(10,2) DEFAULT NULL COMMENT '最低原价/划线价',
  `max_original_price` decimal(10,2) DEFAULT NULL COMMENT '最高原价/划线价',
...
UNLOCK TABLES;
-- This dump is a V12 baseline. Remove columns that were accidentally exported
-- from later migrations so Flyway can apply V13+ in order.
ALTER TABLE product
    DROP INDEX idx_status_recommend_sort,
    DROP COLUMN is_recommend,
    DROP COLUMN min_original_price,
    DROP COLUMN max_original_price;
ALTER TABLE product_sku DROP COLUMN original_price;

-- Never ship known bootstrap credentials or legacy WeChat secrets in a dump.
UPDATE admin_user SET password_hash = '!disabled-default-account!' WHERE username = 'admin';
UPDATE merchant_user SET password_hash = '!disabled-default-account!' WHERE username = 'merchant01';
UPDATE merchant SET wx_app_id = '', wx_secret = '';
