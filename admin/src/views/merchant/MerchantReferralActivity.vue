...
import { couponTemplateApi, referralApi, type CouponPurpose, type CouponTemplate, type ReferralCampaign, type ReferralCampaignPayload, type ReferralRelation, type ReferralReward, type ReferralStats } from '@/api/marketing'
...
const hasPurpose = (item: CouponTemplate, purpose: CouponPurpose) => (item.purposeCodes || []).includes(purpose)
const inviteeTemplates = computed(() => activeTemplates.value.filter((item) => hasPurpose(item, 'REFERRAL_INVITEE')))
const inviterTemplates = computed(() => activeTemplates.value.filter((item) => hasPurpose(item, 'REFERRAL_INVITER')))

async function load() {
  loading.value = true
  try {
    const [campaignList, templateList] = await Promise.all([referralApi.list(), userStore.hasPermission('merchant:coupon:view') ? couponTemplateApi.list() : Promise.resolve([])])
...
<template>
  <div class="page referral-page">
    <div class="page-header"><div><span class="page-kicker">REFERRAL GROWTH</span><h1 class="page-title">分享有礼</h1><p class="page-desc">邀请新用户完成首单，按 1 / 3 / 5 人阶梯发放店铺优惠券。活动开关在“营销活动”中控制。</p></div><div class="header-actions"><el-button @click="load">刷新</el-button><el-button v-permission="'merchant:referral:create'" type="primary" :disabled="!userStore.hasPermission('merchant:coupon:view')" @click="openCreate">新建活动</el-button></div></div>
