...
interface Props {
  ...
  parent?: MerchantCategoryVO | null
  current?: MerchantCategoryVO | null
}
...
async function submit() {
  ...
  try {
    if (props.mode === 'edit' && props.current) {
      await merchantCategoryApi.update(props.current.id, { ...form })
      ElMessage.success('已更新')
    } else {
      await merchantCategoryApi.create({ ...form })
  ...
  >
    <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
      ...
      <el-form-item label="分类图标">
        <ImageUploader v-model="form.icon" scope="merchant" :limit="1" label="上传图标" />
      </el-form-item>
    </el-form>
    <template #footer>
