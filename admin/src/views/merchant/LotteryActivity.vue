...
async function load() {
  ...
  try {
    const [list, couponList, productPage] = await Promise.all([
      lotteryApi.list(),
      couponTemplateApi.list(),
      productApi.page({ page: 1, size: 200, status: 1, auditStatus: 1 }),
    ])
...
<template>
  <div class="lottery-page page">
    <div class="page-header"><div><span class="page-kicker">DAILY SURPRISE</span><h1 class="page-title">互动抽奖盲盒</h1><p class="page-desc">配置活动时间、参与门槛和透明概率；系统保证同一商家同一时间仅一个抽奖活动在线。</p></div><div class="page-actions"><el-button @click="load">刷新</el-button><el-button type="primary" @click="openCreate">新建活动</el-button></div></div>
