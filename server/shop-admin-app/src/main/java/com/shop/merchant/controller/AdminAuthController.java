...
public class AdminAuthController {
    ...
    public ApiResult<LoginResponse> login(@RequestBody @Valid LoginRequest req) {
        return ApiResult.success(adminUserService.login(req.getUsername(), passwordCipher.decrypt(req.getPassword())));
    }

    @PutMapping("/password")
    public ApiResult<Void> changePassword(@RequestBody @Valid ChangePasswordRequest req) {
        adminUserService.changePassword(CurrentUserHolder.get().getUserId(),
