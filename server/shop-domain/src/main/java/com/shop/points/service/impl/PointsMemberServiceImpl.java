...
public class PointsMemberServiceImpl implements PointsMemberService {
    ...
    private final CouponService couponService; private final ObjectMapper objectMapper;
    ...
    private PointsProductVO productVO(PointsProduct p, Long uid, Long mid){ PointsProductVO v=new PointsProductVO();v.setId(p.getId());v.setTitle(p.getTitle());v.setImage(p.getImage());v.setPointsPrice(p.getPointsPrice());v.setStock(p.getStock());v.setPerUserLimit(p.getPerUserLimit());v.setPhysical(p.getProductId()!=null); if(uid!=null) v.setRedeemedCount(Math.toIntExact(redeemMapper.selectCount(new LambdaQueryWrapper<PointsRedeemRecord>().eq(PointsRedeemRecord::getUserId,uid).eq(PointsRedeemRecord::getMerchantId,mid).eq(PointsRedeemRecord::getPointsProductId,p.getId()).eq(PointsRedeemRecord::getStatus,1))));return v; }
