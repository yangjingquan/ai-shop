...
public class PointsMemberServiceImpl implements PointsMemberService {
    ...
    @Override @Transactional public void rewardLotteryPoints(Long userId, Long merchantId, int points, String businessNo, String description, int validDays) { if (points > 0) appendLedger(userId, merchantId, points, "LOTTERY_DRAW", businessNo, null, description, validDays); }
    ...
    private PointsProductVO productVO(PointsProduct p, Long uid, Long mid){ PointsProductVO v=new PointsProductVO();v.setId(p.getId());v.setProductId(p.getProductId());v.setSkuId(p.getSkuId());v.setCouponTemplateId(p.getCouponTemplateId());v.setTitle(p.getTitle());v.setImage(displayImage(p));v.setPointsPrice(p.getPointsPrice());v.setStock(p.getStock());v.setPerUserLimit(effectivePerUserLimit(p));v.setStatus(p.getStatus());v.setPhysical(p.getProductId()!=null); if(uid!=null) v.setRedeemedCount(redeemMapper.selectList(new LambdaQueryWrapper<PointsRedeemRecord>().eq(PointsRedeemRecord::getUserId,uid).eq(PointsRedeemRecord::getMerchantId,mid).eq(PointsRedeemRecord::getPointsProductId,p.getId()).eq(PointsRedeemRecord::getStatus,1)).stream().mapToInt(record->safe(record.getQuantity())).sum());return v; }
    private int effectivePerUserLimit(PointsProduct product) {
        int productLimit = safe(product.getPerUserLimit());
        if (product.getCouponTemplateId() == null) return productLimit;
        CouponTemplate template = couponTemplateMapper.selectById(product.getCouponTemplateId());
        int couponLimit = template == null ? 0 : safe(template.getPerUserLimit());
        if (couponLimit <= 0) return productLimit;
        return productLimit <= 0 ? couponLimit : Math.min(productLimit, couponLimit);
    }
