...
public interface OrderMapper extends BaseMapper<Order> {
    ...
    @Select("SELECT * FROM `order` WHERE status = 0 AND order_type <> 6 AND created_at < DATE_SUB(NOW(), INTERVAL 30 MINUTE) ORDER BY id ASC LIMIT #{limit}")
    ...
    @Update("UPDATE `order` SET status = 2, fulfillment_method = #{fulfillmentMethod}, ship_company = #{shipCompany}, shipper_code = #{shipperCode}, ship_no = #{shipNo}, ship_time = #{now}, updated_at = #{now} " +
            "WHERE order_no = #{orderNo} AND merchant_id = #{merchantId} AND deleted = 0 " +
            "AND ((order_type = 0 AND status = 1) OR (order_type = 1 AND status = 6) OR (order_type = 2 AND status = 1) OR (order_type = 3 AND status = 1) OR (order_type = 4 AND status = 1) OR (order_type = 5 AND status = 1) OR (order_type = 6 AND status = 1)) " +
    ...
    int ship(@Param("merchantId") Long merchantId, @Param("orderNo") String orderNo,
             @Param("fulfillmentMethod") Integer fulfillmentMethod,
