...
public interface OrderMapper extends BaseMapper<Order> {
    ...
    @Update("UPDATE `order` SET status = 2, ship_company = #{shipCompany}, shipper_code = #{shipperCode}, ship_no = #{shipNo}, ship_time = #{now}, updated_at = #{now} " +
            ...
            "AND ((order_type = 0 AND status = 1) OR (order_type = 1 AND status = 6) OR (order_type = 2 AND status = 1) OR (order_type = 4 AND status = 1) OR (order_type = 5 AND status = 1) OR (order_type = 6 AND status = 1)) " +
