...
public class PromotionServiceImpl implements PromotionService {
    ...
    private void replaceChildren(Long activityId, PromotionActivityRequest r) { thresholdMapper.delete(new LambdaQueryWrapper<PromotionThreshold>().eq(PromotionThreshold::getActivityId, activityId)); scopeMapper.delete(new LambdaQueryWrapper<PromotionScope>().eq(PromotionScope::getActivityId, activityId)); int sort = 0; for (PromotionThresholdRequest t : r.getThresholds()) { PromotionThreshold e = new PromotionThreshold(); e.setActivityId(activityId); e.setThresholdAmount(t.getThresholdAmount()); e.setReductionAmount(t.getReductionAmount()); e.setDiscountRate(t.getDiscountRate()); e.setDiscountCap(t.getDiscountCap()); e.setSort(++sort); thresholdMapper.insert(e); } saveScopes(activityId, SCOPE_CATEGORY, r.getCategoryIds()); saveScopes(activityId, SCOPE_PRODUCT, r.getProductIds()); saveScopes(activityId, SCOPE_EXCLUDED_PRODUCT, r.getExcludedProductIds()); saveScopes(activityId, SCOPE_RECOMMEND_PRODUCT, r.getRecommendProductIds()); }
