...
echo "=== 2. Admin 创建冒烟商家 ==="
SUFFIX=$(date +%s)
CREATE_RESP=$(curl -s -X POST "$ADMIN_BASE/api/admin/merchants" \
...
curl -s -X PUT "$ADMIN_BASE/api/merchant/profile" \
...
curl -s -X POST "$WX_BASE/api/wx/addresses" \
  -H "wx-token: $WTOKEN" -H "Content-Type: application/json" \
...
curl -s "$WX_BASE/api/wx/addresses" -H "wx-token: $WTOKEN" \
