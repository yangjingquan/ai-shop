...
# 使用 dev profile，shop-wx-app 需要在 localhost:8082 运行
set -e
...
hr "Step 1: dev-login"
TOKEN=$(curl -s -X POST "$WX_BASE/api/wx/auth/dev-login" \
...
AID=$(curl -s -X POST "$WX_BASE/api/wx/addresses" \
...
curl -s -X POST "$WX_BASE/api/wx/cart" \
  -H "Content-Type: application/json" -H "wx-token: $TOKEN" \
  --data-binary '{"skuId":9,"quantity":1}' > /dev/null
CID=$(curl -s "$WX_BASE/api/wx/cart" -H "wx-token: $TOKEN" \
  | python3 -c 'import sys,json;print(json.load(sys.stdin)["data"][0]["id"])')
echo "addressId=$AID cartItemId=$CID" && pass "add-cart+address"

hr "Step 3: Preview order"
PREVIEW=$(curl -s -X POST "$WX_BASE/api/wx/order/preview" \
  -H "Content-Type: application/json" -H "wx-token: $TOKEN" \
...
ORDER_NO=$(curl -s -X POST "$WX_BASE/api/wx/order/create" \
  -H "Content-Type: application/json" -H "wx-token: $TOKEN" \
  --data-binary "{\"cartItemIds\":[$CID],\"addressId\":$AID}" \
...
CART_LEN=$(curl -s "$WX_BASE/api/wx/cart" -H "wx-token: $TOKEN" \
...
PAGE_RET=$(curl -s "$WX_BASE/api/wx/order/page?page=1&size=10" -H "wx-token: $TOKEN")
...
DETAIL=$(curl -s "$WX_BASE/api/wx/order/$ORDER_NO" -H "wx-token: $TOKEN")
...
hr "Step 8: Mock pay"
curl -s -X POST "$WX_BASE/api/wx/order/$ORDER_NO/mock-pay" \
  -H "wx-token: $TOKEN" > /dev/null
STATUS=$(curl -s "$WX_BASE/api/wx/order/$ORDER_NO" -H "wx-token: $TOKEN" \
...
curl -s -X POST "$WX_BASE/api/wx/cart" \
  -H "Content-Type: application/json" -H "wx-token: $TOKEN" \
  --data-binary '{"skuId":9,"quantity":1}' > /dev/null
CID2=$(curl -s "$WX_BASE/api/wx/cart" -H "wx-token: $TOKEN" \
  | python3 -c 'import sys,json;print(json.load(sys.stdin)["data"][0]["id"])')
ON2=$(curl -s -X POST "$WX_BASE/api/wx/order/create" \
  -H "Content-Type: application/json" -H "wx-token: $TOKEN" \
  --data-binary "{\"cartItemIds\":[$CID2],\"addressId\":$AID}" \
  | python3 -c 'import sys,json;print(json.load(sys.stdin)["data"][0]["orderNo"])')
curl -s -X POST "$WX_BASE/api/wx/order/$ON2/cancel" \
  -H "wx-token: $TOKEN" > /dev/null
CANCEL_CODE=$(curl -s -X POST "$WX_BASE/api/wx/order/$ON2/cancel" \
