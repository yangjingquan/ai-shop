...
Page({
  ...
  load() { this.setData({ loading: true }); return Promise.all([pointsApi.profile(), pointsApi.ledger(), lotteryApi.current().catch(() => null)]).then(([p, l, lottery]) => {
    ...
    profile.hasNextLevel = profile.nextLevelPoints !== null && profile.nextLevelPoints !== undefined
    profile.pointsToNext = profile.hasNextLevel ? Math.max(0, Number(profile.nextLevelPoints) - Number(profile.totalPoints || 0)) : null
    ...
    this.setData({ profile, ledgers, signedToday, lotteryActivity: lottery && lottery.data || null, loading: false })
