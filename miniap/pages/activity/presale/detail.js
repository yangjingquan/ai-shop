...
Page({
  ...
  load() { this.setData({ loading: true }); return presaleApi.detail(this.activityId).then((res) => { const activity = res && res.data; if (!activity) throw new Error('missing activity'); activity.skus = (activity.skus || []).map((sku) => ({ ...sku, mainImage: resolveImageUrl(sku.mainImage || ''), depositText: this.fmt(sku.depositAmount), deductionText: this.fmt(sku.depositDeductionAmount), balanceText: this.fmt(sku.balanceAmount), finalText: this.fmt(sku.finalAmount) })); const productGroups = groupProducts(activity.skus); activity.cover = resolveImageUrl(activity.bannerImage || productGroups[0]?.mainImage || ''); this.setData({ activity, productGroups, selectedSku: activity.skus[0] || null }) }).catch(() => { wx.showToast({ title:'预售活动加载失败', icon:'none' }) }).finally(() => this.setData({ loading: false })) },
